...
export default function MainLayout({
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-gradient-to-br from-slate-50 via-blue-50/30 to-teal-50/20">
      ...
      <div
        className={`border-b border-slate-200/60 bg-white/80 backdrop-blur-sm transition-all duration-300 ease-in-out ${
          isConfigCollapsed ? "h-14" : "h-60"
        }`}
      >
        <div className="flex h-14 items-center justify-between border-b border-slate-200/50 bg-gradient-to-r from-slate-50 to-blue-50/30 px-6 shadow-sm">
          ...
          >
            ...
            <span>{isConfigCollapsed ? "Expand" : "Collapse"}</span>
          </button>
        </div>
        {!isConfigCollapsed && (
          <div className="h-[calc(100%-3.5rem)] overflow-auto p-6 bg-white/50">{configPanel}</div>
        )}
      </div>
